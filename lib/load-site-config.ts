...
export async function loadSiteConfigFromDB(slug: string): Promise<SiteConfig | null> {
  ...
  // Pull testimonials, gallery (media) — catalog engine has no bookable resources
  ...
  // Services come entirely from sites.config.servicesExtras for catalog engines.
  ...
  const serviceItems: ServiceItem[] = extrasRaw
    .filter((x): x is Record<string, unknown> => typeof x === "object" && x !== null)
    .map((x) => ({
      name: typeof x.name === "string" ? x.name : "",
      description: typeof x.description === "string" ? x.description : "",
      price: typeof x.price === "string" ? x.price : undefined,
      icon: typeof x.icon === "string" ? x.icon : "sparkles",
  ...
  const shopLike =
    get(cfg, "engine") === "catalog" || !!get(cfg, "cartVariant") ||
    /fashion|ecommerce|e-?commerce|shop|store|retail|boutique|apparel|clothing|streetwear/i
      .test(String(site.industry ?? ""));
  ...
  return {
    ...
    services: {
      // Engine-aware fallback: hotel copy must never bleed into shop admins
      // (industry can be "ecommerce"/"boutique"/etc., not just "fashion").
      title:    get(cfg, "services", "title")    || (shopLike ? "Our Collections" : "Our Rooms & Suites"),
      subtitle: get(cfg, "services", "subtitle") || (shopLike ? "Curated styles for every occasion." : "Hand-picked for every kind of stay."),
      items: serviceItems.length > 0 ? serviceItems : [shopLike
        ? { name: "Coming soon", description: "Add products in /admin/products.", icon: "sparkles" }
        : { name: "Coming soon", description: "Add room types in /admin/rooms.", icon: "bed" }],
    },
    gallery: {
...
const TEMPLATE_ID_BY_INDUSTRY: Record<string, string> = {
  fashion: "fashion-v1",
  // Catalog storefronts that don't say "fashion" — built configs commonly
  // carry industry "ecommerce"/"shop" (keep in sync with the host copy in
  // src/lib/tenantProvisioner.ts).
  ecommerce: "fashion-v1",
  "e-commerce": "fashion-v1",
  shop: "fashion-v1",
  store: "fashion-v1",
  retail: "fashion-v1",
  boutique: "fashion-v1",
  streetwear: "fashion-v1",
  womenswear: "fashion-v1",
  menswear: "fashion-v1",
...
function templateIdForIndustry(industry: string | null | undefined): string {
  ...
  if (!raw) return "";
  const exact = TEMPLATE_ID_BY_INDUSTRY[raw];
  if (exact) return exact;
  // Free-text industries ("ecommerce (fashion / womenswear)") never
  // exact-match — per-token lookup, same as the host-side resolver.
  for (const token of raw.split(/[^a-z0-9-]+/)) {
    const hit = TEMPLATE_ID_BY_INDUSTRY[token];
    if (hit) return hit;
  }
  return "";
}

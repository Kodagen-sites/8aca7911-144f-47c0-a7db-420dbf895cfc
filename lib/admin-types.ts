...
export function getIndustryModule(
  ...
  const byTemplate = moduleFromTemplateId(cfg.templateId ?? "");
  // Shape evidence beats the generic fallback: a config that carries a cart
  // variant or a products array IS a storefront, even when none of the
  // stamping keys survived (pre-fix provisions, manual config writes). A
  // shop must never open on an appointments/hotel admin (Kessa 2026-07-15).
  if (!cfg.templateId && !engine) {
    const shape = cfg as { cartVariant?: unknown; products?: unknown };
    if (shape.cartVariant || Array.isArray(shape.products)) return "catalog";
  }
  return byTemplate;
}

...
type OrderRow = {
  ...
  itemCount: number; items: OrderItem[]; createdAt: string; paymentProvider: string;
  paidAt: string | null;
};
...
const stateConfig: Record<string, { label: string; dot: string; text: string; bg: string; ring: string; icon: React.ComponentType<{ className?: string }> }> = {
  ...
  paid:       { label: "Paid",       dot: "bg-green-400",  text: "text-green-600",  bg: "bg-green-500/10",  ring: "ring-green-500/20",  icon: CreditCard },
...
const NEXT_ACTIONS: Record<string, { state: string; label: string; primary?: boolean }[]> = {
  ...
  paid:       [{ state: "processing", label: "Start Processing", primary: true }, { state: "cancelled", label: "Cancel" }],
...
function OrdersContent({ orders }: Omit<Props, "config">) {
  ...
  return (
      <div className="p-5 sm:p-6 lg:p-8 space-y-6 max-w-6xl mx-auto">
        ...
        {/* Stats */}
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          {[
            { label: "Pending", value: orders.filter((o) => o.state === "pending").length, color: "#f59e0b", icon: Clock },
            { label: "Paid", value: orders.filter((o) => o.state === "paid").length, color: "#22c55e", icon: CreditCard },
            { label: "Processing", value: orders.filter((o) => ["processing", "confirmed", "preparing"].includes(o.state)).length, color: "#6366f1", icon: Box },
            { label: "Shipped", value: orders.filter((o) => ["shipped", "ready"].includes(o.state)).length, color: "#a855f7", icon: Truck },
            { label: "Delivered", value: orders.filter((o) => ["delivered", "completed"].includes(o.state)).length, color: "#10b981", icon: CheckCircle2 },
          ].map((stat, i) => {
              ...
              <div className={`p-4 rounded-xl ${s.sectionBg} space-y-3`}>
                ...
                <div className={`pt-3 border-t ${s.borderLight} space-y-2`}>
                  {[
                    ...
                    ...(selected.paidAt ? [{ label: "Paid", value: fmtDate(selected.paidAt) }] : []),
              ...
              {selected.items.length > 0 && (
                <div className={`p-4 rounded-xl ${s.sectionBg} space-y-3`}>
                  <p className={`text-[10px] font-semibold uppercase tracking-wider ${s.textMuted}`}>
                    Items ({selected.items.reduce((n, it) => n + (it.qty || 1), 0)})
                  </p>
                  <div className={`divide-y ${s.divider}`}>
                    {selected.items.map((it, idx) => (
                      <div key={idx} className="flex items-center justify-between gap-3 py-2 first:pt-0 last:pb-0">
                        <div className="min-w-0">
                          <p className={`text-sm ${s.textPrimary} truncate`}>
                            <span className="font-semibold">×{it.qty || 1}</span> {it.name}
                          </p>
                          {it.variant && <p className={`text-xs ${s.textMuted}`}>{it.variant}</p>}
                        </div>
                        <span className={`text-sm font-medium flex-shrink-0 ${s.textSecondary}`}>
                          {m.money(((it.price || 0) * (it.qty || 1)) / 100, selected.currency)}
                        </span>
                      </div>
                    ))}
                  </div>

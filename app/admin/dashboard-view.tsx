...
export type DashboardData = {
  /** catalog = shop admin (orders/products); booking = everything else */
  ...
  stats: {
    totalRevenue: number;
    revenueToday: number;
    /** catalog mode: orders placed today */
...
function DashboardContent({ data }: { data: DashboardData }) {
  ...
  const catalog = data.mode === "catalog";
  ...
  // Every tile navigates to the page it summarizes.
  type StatCard = {
    label: string; value: number; prefix?: string; suffix?: string;
    icon: React.ComponentType<{ className?: string }>; trend: string; color: string; href: string;
  };
  const statCards: StatCard[] = catalog ? [
    { label: "Revenue Today",  value: stats.revenueToday, prefix: CURRENCY_SYMBOL, icon: DollarSign,    trend: "Today",    color: "#22c55e", href: "/admin/transactions" },
    { label: "Orders Today",   value: stats.bookingsToday,                icon: Package,       trend: "Today",    color: "#3b82f6", href: "/admin/orders" },
    { label: "Paid Orders",    value: stats.paidOrders,                   icon: CheckCircle,   trend: "All time", color: "#a855f7", href: "/admin/orders" },
    { label: "Total Revenue",  value: stats.totalRevenue, prefix: CURRENCY_SYMBOL, icon: TrendingUp,    trend: "All paid", color: "#f59e0b", href: "/admin/reports" },
    { label: "New Inquiries",  value: stats.newInquiries,                 icon: MessageSquare, trend: "Unread",   color: "#ec4899", href: "/admin/inquiries" },
    { label: "Products",       value: stats.productCount,                 icon: ShoppingBag,   trend: "In shop",  color: "#06b6d4", href: "/admin/products" },
    { label: "Low Stock",      value: stats.lowStock,                     icon: Warehouse,     trend: "≤3 left",  color: "#ef4444", href: "/admin/inventory" },
  ] : [
    { label: "Revenue Today",   value: stats.revenueToday,   prefix: CURRENCY_SYMBOL, icon: DollarSign,    trend: "Today",          color: "#22c55e", href: "/admin/transactions" },
    { label: "Bookings Today",  value: stats.bookingsToday,                icon: CalendarCheck, trend: "Today",          color: "#3b82f6", href: "/admin/bookings" },
    { label: "Checked In",      value: currentGuests.length,               icon: BedDouble,     trend: "Now",            color: "#a855f7", href: "/admin/bookings" },
    { label: "Total Revenue",   value: stats.totalRevenue,   prefix: CURRENCY_SYMBOL, icon: TrendingUp,    trend: "All paid",       color: "#f59e0b", href: "/admin/reports" },
    { label: "New Inquiries",   value: stats.newInquiries,                  icon: MessageSquare, trend: "Unread",         color: "#ec4899", href: "/admin/inquiries" },
    { label: "Events Booked",   value: eventBookings,                       icon: Eye,           trend: "Recent",         color: "#06b6d4", href: "/admin/bookings" },
    { label: "Conversion",      value: stats.conversionRate, suffix: "%",  icon: Activity,      trend: "—",      color: "#10b981", href: "/admin/reports" },
  ];

  // Only show charts that have actual data
  const allCharts = [
    { label: `Revenue (${CURRENCY_SYMBOL}) — Last 7 Days`, data: data.charts.revenue7d,   color: "#22c55e" },
    { label: `${catalog ? "Orders" : "Bookings"} — Last 7 Days`,    data: data.charts.bookings7d,  color: "#3b82f6" },
  ];
  const charts = allCharts.filter((c) => c.data.some((v) => v > 0));

  const systemHealth = [
    { name: "Website", status: "Active", ok: true },
    { name: catalog ? "Checkout & Orders" : "Booking System", status: "Active", ok: true },
  ...
  const orderStatusDot: Record<string, { label: string; dot: string; text: string }> = {
    pending: { label: "Pending", dot: "bg-amber-400", text: dark ? "text-amber-400" : "text-amber-600" },
    paid: { label: "Paid", dot: "bg-green-400", text: dark ? "text-green-400" : "text-green-600" },
    preparing: { label: "Preparing", dot: "bg-blue-400", text: dark ? "text-blue-400" : "text-blue-600" },
    processing: { label: "Processing", dot: "bg-blue-400", text: dark ? "text-blue-400" : "text-blue-600" },
    ready: { label: "Ready", dot: "bg-purple-400", text: dark ? "text-purple-400" : "text-purple-600" },
    shipped: { label: "Shipped", dot: "bg-purple-400", text: dark ? "text-purple-400" : "text-purple-600" },
    delivered: { label: "Delivered", dot: "bg-gray-400", text: dark ? "text-gray-500" : "text-gray-500" },
    completed: { label: "Completed", dot: "bg-gray-400", text: dark ? "text-gray-500" : "text-gray-500" },
    cancelled: { label: "Cancelled", dot: "bg-red-400", text: dark ? "text-red-400" : "text-red-600" },
  };
    ...
    <div className="p-5 sm:p-6 lg:p-8 space-y-6 overflow-hidden">
      ...
      >
        ...
        <div className="relative flex-1 max-w-md">
          ...
          <input
            type="text"
            placeholder={catalog ? "Search orders, products, customers..." : "Search bookings, guests, inquiries..."}
          ...
          return (
            <Link key={stat.label} href={stat.href} className="block">
              <motion.div
                ...
                className={`${cardBg} rounded-2xl border ${cardBorder} p-4 h-full cursor-pointer transition-all ${dark ? "hover:border-white/20" : "hover:border-gray-300 hover:shadow-sm"}`}
              >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        >
          <div className="flex items-center justify-between p-5">
            <h2 className={`font-bold ${textPrimary}`}>{catalog ? "Recent Orders" : "Recent Bookings"}</h2>
            <Link href={catalog ? "/admin/orders" : "/admin/bookings"} className="text-xs font-semibold text-blue-400 hover:text-blue-300 transition-colors flex items-center gap-1">
          ...
          <div className={`divide-y ${dark ? "divide-white/[0.04]" : "divide-gray-100"}`}>
            {recentOrders.length === 0 && (
              <p className={`px-5 py-6 text-sm ${textMuted}`}>No orders yet — they land here the moment checkout completes.</p>
            )}
            {recentOrders.map((o, i) => {
              const od = orderStatusDot[o.status] || (o.paid ? orderStatusDot.paid : orderStatusDot.pending);
              return (
                <motion.div
                  key={o.id}
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className={`text-sm font-semibold truncate ${textPrimary}`}>{o.customerName}</p>
                  ...
                  <div className="flex items-center gap-3 flex-shrink-0">
                    <span className={`text-sm font-semibold hidden sm:block ${textSecondary}`}>
                      {CURRENCY_SYMBOL}{(o.totalCents / 100).toLocaleString()}
                    </span>
                    <span className={`flex items-center gap-1.5 text-[11px] font-medium ${od.text}`}>
                      <span className={`w-1.5 h-1.5 rounded-full ${od.dot}`} />
                      {od.label}
                    </span>
                  </div>
                </motion.div>
      ...
      {/* Room Occupancy Bar — booking engines only */}
      {!catalog && (
      <motion.div
        initial={{ opacity: 0, y: 15 }}
        animate={{ opacity: 1, y: 0 }}

...
// Minimal stub — the SDK isn't installed on tenant deployments; keyless callers
// route through the platform proxy. The direct-SDK branch below is dead code
// on this build but kept for editorial clarity of the fallback shape.
type AnthropicClient = { messages: { create: (args: { model: string; max_tokens: number; system: string; messages: { role: string; content: string }[] }) => Promise<{ content: { type: string; text?: string }[] }> } };
const Anthropic = function (_: { apiKey: string }): AnthropicClient {
  throw new Error("Anthropic SDK not installed on tenant deployments");
} as unknown as new (opts: { apiKey: string }) => AnthropicClient;

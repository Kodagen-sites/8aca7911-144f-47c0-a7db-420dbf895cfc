...
export default async function TeamPage() {
  ...
  const memberIds = ((mappings ?? []) as MappingRow[]).map((m) => m.user_id as string);
  ...
  const members: TeamMember[] = ((mappings ?? []) as MappingRow[]).map((m) => {
  ...
  const audit = ((auditRows ?? []) as AuditRow[]).map((a) => ({
  ...
  return <TeamView members={members} audit={audit} currentRole={ctx.role} config={config as SiteConfig} counts={counts} />;
}

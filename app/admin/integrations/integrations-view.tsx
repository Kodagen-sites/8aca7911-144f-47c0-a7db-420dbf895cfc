...
import { getIndustryModule } from "@/lib/admin-types";
...
// Engine-aware catalog: a shop's admin must speak checkout/orders, a booking
// business reservations, a CRM leads. One definition list, tailored per the
// site's industry module — never booking copy on a catalog site.
function buildCatalog(industryMod: string): IntegrationDef[] {
  const isCatalog = industryMod === "catalog";
  const noun = isCatalog ? "order" : industryMod === "tickets" ? "registration" : industryMod === "crm" ? "lead" : "booking";
  const nounCap = noun.charAt(0).toUpperCase() + noun.slice(1);
  const paySurface = isCatalog ? "checkout" : "booking page";
  ...
  // ─── Payments ────
  {
    ...
    fields: [
      ...
      { key: "public_key",   label: "Public Key",      type: "text",     required: true, placeholder: "pk_test_…  or  pk_live_…",
        help: `Used by the ${paySurface} to tokenise cards. Safe to expose.` },
  ...
  // ─── Email ────
  {
    kind: "email",
    name: "Email (Resend)",
    tagline: `${nounCap} confirmations & notifications`,
    description: `Automatically send ${noun} confirmations, inquiry auto-replies, and admin notifications. Managed by Kodagen by default — add your own Resend API key for a custom sending domain.`,
    icon: Mail,
    color: "#000000",
    category: "Messaging",
  ...
  // ─── Google ────
  {
    kind: "google_sheets",
    name: "Google Sheets",
    tagline: industryMod === "crm" ? "Leads & contacts → spreadsheet" : `${nounCap}s & leads → spreadsheet`,
    description: `Every new ${noun} and inquiry is appended as a row in your spreadsheet — a live log your team can filter, share, and build on.`,
  ...
  // Calendar is a scheduling tool — a catalog shop has nothing to put on it.
  return isCatalog ? defs.filter((d) => d.kind !== "google_calendar") : defs;
}

const CATEGORY_ORDER: IntegrationDef["category"][] = ["Payments", "Analytics", "Messaging", "Google", "Custom"];

// ─── Which integrations exist on this site ────────────
// STRICT: only what the customer picked renders — an unpicked provider must
// never appear on the page at all (owner directive 2026-07-15: no "browse
// more", no pitching Stripe to a Paystack business). Resolution order:
//   1. config.enabledIntegrations — stamped into sites.config at
//      provisioning from the feature-gate answers; DB-read at request time
//      so it can't go stale like a boot-time env var.
//   2. NEXT_PUBLIC_ENABLED_INTEGRATIONS — older builds (baked at boot).
//   3. Derived from the config itself — payment provider / WhatsApp / chat
//      the built site actually uses.
// Anything with a stored row (already configured) always stays visible.
function enabledKindsFor(config: SiteConfig): Set<string> {
  const fromConfig = (config as { enabledIntegrations?: unknown }).enabledIntegrations;
  if (Array.isArray(fromConfig) && fromConfig.length > 0) {
    return new Set(fromConfig.map((k) => String(k).trim().toLowerCase()).filter(Boolean));
  }
  const fromEnv = (process.env.NEXT_PUBLIC_ENABLED_INTEGRATIONS ?? "")
    .split(",").map((k) => k.trim().toLowerCase()).filter(Boolean);
  if (fromEnv.length > 0) return new Set(fromEnv);
  const blob = JSON.stringify(config ?? {}).toLowerCase();
  const derived = new Set<string>();
  if (blob.includes("paystack")) derived.add("paystack");
  else if (blob.includes("stripe")) derived.add("stripe");
  if (blob.includes("whatsapp")) derived.add("whatsapp");
  if (blob.includes("tawk")) derived.add("tawkto");
  derived.add("email"); // transactional mail ships with every admin
  return derived;
}
...
function IntegrationForm({
  ...
  // autoComplete="off" + non-credential names + password-manager opt-outs:
  // Chrome treats a text input followed by a password input as a login form
  // and injects the saved admin email into it (the "public key prefilled
  // with admin@…" bug) — autocomplete must be defeated at the form level.
  return (
    <form action={action} autoComplete="off" className={`${s.cardBg} rounded-2xl border ${s.cardBorder} p-6 space-y-5`}>
...
function IntegrationsInner({
  ...
  const isCatalogEngine = industryMod === "catalog";
  const CATALOG = useMemo(() => buildCatalog(industryMod), [industryMod]);

  // STRICT visibility: picked (or already-configured) integrations only.
  // Everything else does not exist on this page.
  const enabled = useMemo(() => enabledKindsFor(config), [config]);
  const visible = CATALOG.filter((def) => enabled.has(def.kind) || Boolean(stored[def.kind]));
            ...
            <div className={`mt-4 ${s.cardBg} rounded-2xl border ${s.cardBorder} p-4 flex items-start gap-3`}>
              ...
              <div className="text-xs space-y-1">
                <p className={`font-semibold ${s.textPrimary}`}>
                  How payments hook up to {isCatalogEngine ? "checkout" : "bookings"}
                </p>
                <p className={s.textMuted}>
                  {isCatalogEngine
                    ? <>Once enabled with valid keys, checkout initialises a {activeDef.name} payment before the order is placed.</>
                    : <>Once enabled with valid keys, the booking modal initialises a {activeDef.name} payment before the reservation is confirmed.</>
                  } The webhook URL above tells {activeDef.name} where

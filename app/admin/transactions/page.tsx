...
export default async function TransactionsPage() {
  ...
  type BookingRow = { reference: string; resource_id: string };
  ...
  const { data: resourceRows } = resourceIds.size > 0
    ...
    : { data: [] };
  type ResourceRow = { id: string; name: string; type: string };
  const resourceById = new Map(((resourceRows ?? []) as ResourceRow[]).map((r) => [r.id as string, r]));
  ...
  const transactions: AdminTransaction[] = ((rows ?? []) as TxRow[]).map((r) => {
  ...
  return <TransactionsView transactions={transactions} config={config as SiteConfig} counts={counts} />;
}
